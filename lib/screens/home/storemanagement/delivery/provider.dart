...
class DeliveryNotifier extends _$DeliveryNotifier {
  ...
  void getDeliveryTakeoutInfo() async {
    final response = await ref
        ...
        .getDeliveryTakeoutInfo(storeId: UserHive.getBox(key: UserKey.storeId));
  ...
  void updateDeliveryTime(int minDeliveryTime, int maxDeliveryTime) async {
    final response = await ref.read(deliveryServiceProvider).updateDeliveryTime(
  ...
  void updatePickupTime(int minTakeOutTime, int maxTakeOutTime) async {
    final response = await ref.read(deliveryServiceProvider).updatePickupTime(
        storeId: UserHive.getBox(key: UserKey.storeId),
        minTakeOutTime: minTakeOutTime,
        maxTakeOutTime: maxTakeOutTime);
    if (response.statusCode == 200) {
      state = state.copyWith(
          error: const ResultFailResponseModel(),
          minTakeOutTime: minTakeOutTime,
  ...
  /// POST - 가게 배달팁 변경
  void updateDeliveryTip(int baseDeliveryTip, int minimumOrderPrice,
      List<DeliveryTipModel> storeDeliveryTipDTOList) async {
    final response = await ref.read(deliveryServiceProvider).updateDeliveryTip(
        storeId: UserHive.getBox(key: UserKey.storeId),
        baseDeliveryTip: baseDeliveryTip,
...
abstract class DeliveryState with _$DeliveryState {
  const factory DeliveryState({
    ...
    @Default(0)
    int deliveryTipMax, // 기본 배달팁 최대금액. figma 상 존재하나, api 규격에 존재하지 않음.
    @Default(0)
    int minimumOrderPrice, // 최소 주문 금액. figma 상 존재하나, api 규격에 존재하지 않음.
    @Default(<DeliveryTipModel>[])
    List<DeliveryTipModel> storeDeliveryTipDTOList,
    @Default('') String deliveryAddress,
    @Default(ResultFailResponseModel()) ResultFailResponseModel error,
  }) = _DeliveryState;

  factory DeliveryState.fromJson(Map<String, dynamic> json) =>
      _$DeliveryStateFromJson(json);
}
