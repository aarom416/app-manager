...
class _OperationScreenState extends ConsumerState<OperationScreen> {
  ...
  void showFailDialogWithImageBoth(
      OperationNotifier provider, int type, String mainTitle, String subTitle) {
  ...
  Widget build(BuildContext context) {
    ...
    final OperationNotifier provider =
        ref.read(operationNotifierProvider.notifier);

    final List<OperationTimeDetailModel> regularHolidays = state
        .holidayDetailDTOList
        .where((holiday) => holiday.isRegularHoliday())
        .toList()
      ..sort((a, b) => a.cycle.compareTo(b.cycle));
    sortRegularHolidays(regularHolidays);

    final List<OperationTimeDetailModel> temporaryHolidays = state
        .holidayDetailDTOList
        .where((holiday) => holiday.isTemporaryHoliday())
        .toList()
    ...
    final List<String> regularHolidayLabels =
        groupRegularHolidaysByCycle(regularHolidays);

    final List<List<OperationTimeDetailModel>>
        groupedOperationTimeDetailDTOList = groupByStartAndEndTime(
            state.operationTimeDetailDTOList, regularHolidays);

    final List<List<OperationTimeDetailModel>> groupedBreakTimeDetailDTOList =
...
List<OperationTimeDetailModel> fillMissingDays(
    List<OperationTimeDetailModel> data) {
...
List<String> groupRegularHolidaysByCycle(
    List<OperationTimeDetailModel> regularHolidays) {
